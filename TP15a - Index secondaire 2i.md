...
____
```text
Sans index, la clause "WHERE" doit préciser le noeud visé
et donc impérativement indiquer la (ou les) colonne(s) de la clé de partitionnement 
...
CREATE INDEX index_intitule_cours ON entrepriseformation.cours (intitule);
```
